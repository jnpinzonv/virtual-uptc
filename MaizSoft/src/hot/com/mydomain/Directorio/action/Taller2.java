...
public class Taller2 implements Taller {
	...
	public Long crearResgistro() {
		 ...
		 * userA.setDocumentoIdentidad(1l); userA.setApellidos("apellidos");
		 * userA.setEnabled(true); userA.setPasswordHash("");
		 ...
		 * escuela.setNombreEscuela("sistemas"); escuela.setIdEscuela((long)
		 * 71);
		 * 
		 * listaEscuelas.add(escuela); GrupoCurso grupoCurso = new GrupoCurso();
		 * LinkedList<Curso> listaCursos = new LinkedList<Curso>();
		 * LinkedList<Curso> listaCursos1 = new LinkedList<Curso>();
		 * 
		 * Curso curso = new Curso(); curso.setCodigo((long) 81671);
		 * curso.setEscuelaCurso(escuela); //curso.set Set<UserAccount>
		 * nuevoMensaje = new HashSet<UserAccount>(); Set<GestorMensajeria>
		 * nuevoM = new HashSet<GestorMensajeria>();
		 * 
		 * GestorMensajeria gestorMensajeria = new GestorMensajeria();
		 * gestorMensajeria.setAsunto("hola"); gestorMensajeria
		 ...
		 * 
		 * nuevoM.add(gestorMensajeria); //entityManager.persist(userA);
		 * 
		...
		/*
		 * Tipo tipo = new Tipo(); tipo.setNombre("facultad");
		 * tipo.setTipo(null); entityManager.persist(tipo); tipo = new Tipo();
		 ...
		 * Tipo.class, 1l)); enteUniversitario.setEnteUniversitario(null);
		...
		grupoUsuarios.setUserGrupoCurso(entityManager.find(Usuario.class, 1l));
		...
		// 2l));
		// grupoUsuarios.setGrupoCurso(entityManager.find(GrupoCurso.class,
		// 1l));
		// System.out.println(userAction.getUsername()+"hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh");
		/*
		 * Usuario userA = new Usuario(); userA.setPrimerNombre("primerNombre");
		 * userA.setCorreoElectronico("correoElectronico@hotmail.co");
		 * userA.setDocumentoIdentidad(1l); userA.setApellidos("apellidos"); /*
		 * userA.setEnabled(true); userA.setPasswordHash("1");
		 * userA.setPassword("1"); userA.setConfirm("1");
		 ...
		 * , 1L)); userA.setTipo(entityManager.find(Tipo.class, 3L));
		 ...
		 * "select t from EnteUniversitario t where t.tipoEnteUniversitario=19"
		 * ); List<EnteUniversitario> listaTiposEnteUniversitarios =
		 * q.getResultList();
		 * facesMessages.add(listaTiposEnteUniversitarios.get(0).getClass()+"");
		 * for (int i = 0; i < 2; i++) {
		 * facesMessages.add(listaTiposEnteUniversitarios
		 * .get(0).getNombreEnteUniversitario()); }
		 */

		Query l = entityManager
				.createQuery("SELECT us FROM UserAccount u JOIN u.roles r,EnteUniversitario e,Usuario us,CuentasUsuario c"
						+ " where r.name='estudiante'"
						+ " and u.id = c.id"
						+ " and c.id = us.id"
						+ " and us.enteUniversitarios.idEnteUniversitario =15");
		// "select u from UserAccount u JOIN u.roles t where t.name='estudiante'"
		List<Usuario> u = l.getResultList();
		System.out.println(u.get(0).getApellidos() + " hola");
		return 1L;
	}
