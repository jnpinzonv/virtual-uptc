...
import com.mydomain.Directorio.chat.UsersBD;
...
public class ChatHelper implements Runnable {
				...
				if (onlineUsers.size() > 0) {
					...
					for (int i = 0; i < onlineUsers.size(); i++) {
						...
						while (keys.hasMoreElements()) {
							...
							if (onlineUsers.get(current).getTime()
									+ ChatConfig.expireLoginAfter < new Date()
									.getTime()) {
								logOffUser(current);
							}
						}
	...
	public int getUserStatus(int userId) {
		if (isUserOnline(userId))
			return ChatUser.ONLINE;
		else
			return ChatUser.OFFLINE;
	}
	/**
	...
	public ChatUser authenticateUser(String username, String password) {
		for (int i = 0; i < usersList.size(); i++) {
			if (usersList.get(i).getLogin().equals(username)
					&& usersList.get(i).getPassword().equals(password)) {
