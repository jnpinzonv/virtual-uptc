...
import com.mydomain.Directorio.chat.ChatHelper;
	...
	protected void processRequest(HttpServletRequest request,
		...
		if (stepId == null || stepId.equals("")) {
			...
			if (username == null || password == null) {
				...
				ChatUser loginUser = chatHelper.authenticateUser(username,
						password);
				if (loginUser == null) {
					...
					request.getSession().setAttribute(CONTACT_LIST,
							chatHelper.getContactList(loginUser.getId()));
					request.getRequestDispatcher("/chatboard.jsp").forward(
							request, response);
				}
			}
			...
			chatMessage
					.setMessageTime(cal.get(Calendar.HOUR) + ":"
			...
			ChatMessage[] messages = chatHelper.getUserMessages(Integer
					.parseInt(userId), ChatConfig.RECIEVE_MAX_MESSAGE_PER_CALL);
			if (messages != null && messages.length > 0) {
				...
				try {
					...
					if (ChatConfig.responseType == ChatConfig.XML) {
						...
						for (int i = 0; i < messages.length; i++) {
							String messageAsString = messages[i]
									.toMultipleValues();
			...
			String statusList = "";
			ChatUser[] contactList = (ChatUser[]) request.getSession()
					.getAttribute(CONTACT_LIST);
			if (contactList != null && contactList.length > 0) {
				for (int i = 0; i < contactList.length; i++) {
					int status = chatHelper.getUserStatus(contactList[i]
							.getId());
					contactList[i].setStatus(status);
					statusList += contactList[i].getId() + "ø"
							+ contactList[i].getName() + "ø"
							+ contactList[i].getType() + "ø" + status + "ø";
				}
